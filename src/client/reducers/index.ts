...
import { setQr, setQrScanned, setQrCode } from "../actions";

const qrCodesDefaultState = {} as { [key: string]: string };
const qrCodes = createReducer<typeof qrCodesDefaultState>(
  {},
  qrCodesDefaultState
);
qrCodes.on(setQr, (state, { key, value }) => ({ ...state, [key]: value }));

const qrScannedDefaultState = { error: null, res: null } as {
  error: any;
  res: any;
};
const qrScanned = createReducer<typeof qrScannedDefaultState>(
  {},
  qrScannedDefaultState
);
