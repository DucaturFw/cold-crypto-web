...
function* fetchBlockchainGasInfo() {
  const [ response, error ] = yield call(fetchJson, 'https://cors-anywhere.herokuapp.com/https://ethgasstation.info/json/ethgasAPI.json')
  if (error) throw error
  yield put(setBlockchainGasInfo({ key: 'eth', value: response }))
}

function* fetchBlockchainTicker() {
  const [ response, error ] = yield call(fetchJson, 'https://cors-anywhere.herokuapp.com/https://api.coinmarketcap.com/v1/ticker/ethereum/')
  if (error) throw error
